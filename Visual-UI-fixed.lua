...
local function _safe_call(fn, ...)
    if type(fn) == "function" then
        ...
        if not ok then warn("Callback error: " .. tostring(err)) end
    end
end
...
print("Visual UI Loaded") --// [⚠️ DO NOT DELETE THIS, OR YOUR SCRIPT WILL BE SHUT DOWN!]
...
UserInputService.InputChanged:Connect(function(input: InputObject)
    ...
    function Tab:create_tab()
        ...
        function Module:create_toggle()
			...
			TextLabel.TextWrapped = true
			TextLabel.TextXAlignment = Enum.TextXAlignment.Left
			TextLabel.Text = self.name.name

			if not Library.Flags[self.flag] then
				Library.Flags[self.flag] = self.enabled
			end

			_safe_call(self.callback, Library.Flags[self.flag])
		...
		function Module:create_description_toggle()
			...
			TextLabel.TextWrapped = true
			TextLabel.TextXAlignment = Enum.TextXAlignment.Left
			TextLabel.Text = self.name.description
		...
		function Module:create_paragraph()
			...
			TextLabel.TextWrapped = true
			TextLabel.TextXAlignment = Enum.TextXAlignment.Left
			TextLabel.Text = self.name.title
		end
	...
	_safe_call(self.callback, )
        ...
        self.slider.Box.Glow.UIGradient.Transparency = NumberSequence.new({
          ...
          NumberSequenceKeypoint.new(1, 1)
           ...
           _safe_call(self.callback, number)
        end
        ...
        function Module:create_dropdown()
			...
			dropdown.Box.TextLabel.Text = self.name.name
			...
			function Dropdown:select_option()
				...
				dropdown.Box.TextLabel.Text = self.name.new_option.Text
			end
        ...
        function Module:create_textbox()
			...
			textbox.Box.TextHolder.FocusLost:Connect(function()
				_safe_call(self.callback, textbox.Box.TextHolder.Text)
				Library.save_flags()
			end)

			function Textbox:update(text)
				textbox.Box.TextHolder.Text = text
				_safe_call(self.callback, text)
			end
		...
		function Module:create_keybind()
			...
			keybind.TextLabel.Text = self.name.name
			keybind.Box.TextLabel.Text = self.name.keycode.Name
